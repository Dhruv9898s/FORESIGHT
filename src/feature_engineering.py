...
print(
    df[
        ["sku_id", "week_start", "demand", "lag_1"]
    ].head(10)
)
# Create lag features
for lag in [2, 4, 8]:
...
print(
    df[
        ["sku_id", "week_start", "demand", "rolling_mean_4"]
    ].iloc[0:15]
)
...
calendar["week_start"] = (
    calendar["date"]
    - pd.to_timedelta(calendar["date"].dt.dayofweek, unit="D")
)

# Identify promotional days
calendar["is_promotion"] = (
...
print(
    df[
        ["sku_id", "week_start", "promo_event", "promotion_days"]
    ].head(15)
)
...
sales["discount_pct"] = (
    (sales["list_price"] - sales["unit_price"])
    / sales["list_price"]
) * 100
...
print(
    (df[season_cols].sum(axis=1) == 1).sum(),
    "of",
    len(df)
)
